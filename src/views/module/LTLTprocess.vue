  ...
  <div>
    ...
    <div style="background-color: white;padding: 20px 10px">
      <el-input v-model="queryParams.smeltTimes" size="small" placeholder="请输入计划单号/请输入熔次号/请输入冷轧卷号" :clearable="true">
        <el-button slot="append"	style="background-color: #46b9b9;color: white;line-height: 15px;" size="small" @click="query(queryParams.smeltTimes)">
          查询
        </el-button>
      </el-input>
      <div align="center">
    ...
    <div style="background-color: white;padding: 10px 10px">
      <el-steps :active="active" align-center>
        <el-step title="熔炼工序" description=" " />
        <el-step title="保温工序" description=" " />
        <el-step title="铸轧工序" description=" " />
        <el-step title="出入库记录" description=" " />
      </el-steps>
    </div>
    <el-collapse v-model="activeNames" @change="handleChange">
      <el-collapse-item title="熔炼工序详细信息">
        <div style="background-color: white;padding: 10px 10px">
          <el-table
            :data="rongluData"
            style="width: 100%"
    ...
    <el-collapse v-model="activeNames" @change="handleChange">
      <el-collapse-item title="保温工序详细信息">
        <div style="background-color: white;padding: 10px 10px">
          <el-table
            :data="baowenData"
            style="width: 100%"
    ...
    <el-collapse v-model="activeNames" @change="handleChange">
      <el-collapse-item title="铸轧工序详细信息">
        <div style="background-color: white;padding: 10px 10px">
          <el-table
            :data="zhuzhascData"
            style="width: 100%"
    ...
    <el-collapse v-model="activeNames" @change="handleChange">
      <el-collapse-item title="出入库记录详细信息">
        <div style="background-color: white;padding: 10px 10px">
          <el-table
            :data="zhuzhaChuRu"
            style="width: 100%"
...
import { getCastProduceList, getZhuzhaTimeList } from '@/api/ImdpCastProduce'
...
export default {
  ...
  methods: {
    ...
    async getDoubleLineData() {
      var temp = []
      var tempTime = []
      const { data } = await getZhuzhaTimeList()
      console.log('这是铸轧的数据', data)
      for (const item of data) {
        temp.push(item.param_degasser)
        tempTime.push(item.create_time)
      }
      var list = [{
        type: 'line', symbol: 'circle', symbolSize: [5, 5], data: temp }, {
        type: 'line', symbol: 'circle', symbolSize: [5, 5], data: tempTime }]
      this.doubleLineOption.series = list
      this.doubleLineOption.xAxis.data = tempTime
