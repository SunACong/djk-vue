...
export default {
  ...
  async created() {
     ...
     * 获取一号铸轧机报警历史记录（30条）
     */
    await getListWarnHistoryData({ rollingDeviceNumber: '退火炉3#' }).then((res) => {
  ...
  methods: {
    ...
    getengineList() {
      ...
      //       退火炉3#
      // select * from t_37e21e80a62511ed8b62ff846936488a where ts > now - 5s;
    ...
    getMyHistoryData: function () {
      ...
      getListDuringWarnData({ rollingDeviceNumber: '退火炉3#', rollingName: this.indicatorName, begin: this.begin, end: this.end }).then((res) => {
    ...
    setTimer() {
      if (this.timer == null) {
        this.timer = setInterval(() => {
          ...
          // 定时查询退火炉最新20条报警记录
          getListWarnHistoryData({ rollingDeviceNumber: '退火炉3#', rollingName: this.indicatorName }).then((res) => {
