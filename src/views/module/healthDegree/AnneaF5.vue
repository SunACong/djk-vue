        ...
        <el-card shadow="always">
          <div slot="header" style="line-height: 20px;display: flex;justify-content: space-between;">
            <div style="display: flex;">
              ...
              <span style="line-height: 20px;">5#退火炉参数</span>
            </div>
          </div>
          <div>
...
//5号退货炉
import { getListNewData5, getListSpecial5, rollingOptions, rollingTableData1 } from '@/api/AnneaF'
...
export default {
  ...
  async created() {
     ...
     * 获取退火炉5报警历史记录（30条）
     */
    await getDevice({ rollingDeviceNumber: '退火炉5#' }).then((res) => {
  ...
  methods: {
    ...
    afterList() { 
      ...
      //退火炉5#
      var zong = 'SELECT * FROM t_48b42640a62511ed8b62ff846936488a where ts between'
    ...
    getMyHistoryData() {
      ...
      getListDuringWarnData({ rollingDeviceNumber: '退火炉5#', rollingName: this.indicatorName, begin: this.begin, end: this.end }).then((res) => {
    ...
    getMyData(index, row) {
       ...
       */
        ...
        // console.log(row);
        getListSpecial5({ rollingName: row.rollingName, ts: row.rollingProduceTime }).then((res) => {
    ...
    setTimer() {
      if (this.timer == null) {
        this.timer = setInterval(() => {
          ...
          // 定时查询退火炉最新20条报警记录
          getDevice({ rollingDeviceNumber: '退火炉5#' }).then((res) => {
            this.currentWarnTable = res.data
          })
        }, 3500)
      }
