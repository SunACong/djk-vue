        ...
        <el-card shadow="always">
          <div slot="header" style="line-height: 20px;display: flex;justify-content: space-between;">
            <div style="display: flex;">
              ...
              <span style="line-height: 20px;">7#退火炉参数</span>
            </div>
          </div>
          <div>
...
export default {
  ...
  async created() {
     ...
     * 获取退火炉报警历史记录（30条）
     */
    await getDevice({ rollingDeviceNumber: '退火炉7#' }).then((res) => {
  ...
  methods: {
    ...
    afterList() { 
      ...
      //退火炉7#
      var zong = 'SELECT * FROM t_60cc35b0a62511ed8b62ff846936488a where ts between'
    ...
    getMyHistoryData() {
      ...
      getListDuringWarnData({ rollingDeviceNumber: '退火炉7#', rollingName: this.indicatorName, begin: this.begin, end: this.end }).then((res) => {
    ...
    getMyData(index, row) {
       ...
       */
        ...
        // console.log(row);
        getListSpecial7({ rollingName: row.rollingName, ts: row.rollingProduceTime }).then((res) => {
    ...
    setTimer() {
      if (this.timer == null) {
        this.timer = setInterval(() => {
          ...
          // 定时查询退火炉最新20条报警记录
          getDevice({ rollingDeviceNumber: '退火炉7#' }).then((res) => {
            this.currentWarnTable = res.data
          })
        }, 3500)
      }
