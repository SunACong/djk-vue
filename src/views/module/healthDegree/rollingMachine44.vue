    ...
    <div style="display: flex;margin-top: 5px;">
      <div style="width: 82%">
        <el-card shadow="always">
          ...
          <div>
            <el-table :data="rollingTableData44" stripe style="width: 100%" height="300px" :cell-style="{'text-align':'center','height':'10px','line-hight':'150px'}" :header-cell-style="{'text-align':'center'}"  >
...
import {getListInfo,getListSpecial,rollingTableData44} from "@/api/rollingMachine";
    ...
    await getAvaluateList().then((res) => {
      ...
      this.avaluateList.forEach(item => {
        ...
        if(item.deviceId=="铸轧机工艺参数1"){
          switch (item.name){
            case "上辊电机速度":
              this.rollingTableData44[0].chartData.minData=item.minValue
              this.rollingTableData44[0].chartData.maxData=item.maxValue
              break
            case "下辊电机速度":
              this.rollingTableData44[1].chartData.minData=item.minValue
              this.rollingTableData44[1].chartData.maxData=item.maxValue
              break
            case "主水泵电机速度":
              this.rollingTableData44[2].chartData.minData=item.minValue
              this.rollingTableData44[2].chartData.maxData=item.maxValue
              break
            case "备用水泵电机速度":
              this.rollingTableData44[3].chartData.minData=item.minValue
              this.rollingTableData44[3].chartData.maxData=item.maxValue
              break
            case "卷取电机速度":
              this.rollingTableData44[4].chartData.minData=item.minValue
              this.rollingTableData44[4].chartData.maxData=item.maxValue
              break
          }
        }
      })
        ...
        getListSpecial({rollingName: row.rollingName, rollingProduceTime: row.rollingProduceTime}).then((res) =>{
          ...
          this.listSpecial.forEach(item =>{
            ...
            if(this.rName == "上辊电机速度"){
              this.minData = this.rollingTableData44[0].chartData.minData
              this.maxData =  this.rollingTableData44[0].chartData.maxData
            }else if(this.rName == "下辊电机速度"){
              this.minData = this.rollingTableData44[1].chartData.minData
              this.maxData =  this.rollingTableData44[1].chartData.maxData
            }else if(this.rName == "主水泵电机速度"){
              this.minData = this.rollingTableData44[2].chartData.minData
              this.maxData =  this.rollingTableData44[2].chartData.maxData
            }else if(this.rName == "备用水泵电机速度"){
              this.minData = this.rollingTableData44[3].chartData.minData
              this.maxData =  this.rollingTableData44[3].chartData.maxData
            }else if(this.rName == "卷取电机速度"){
              this.minData = this.rollingTableData44[4].chartData.minData
              this.maxData =  this.rollingTableData44[4].chartData.maxData
            }
          })
      ...
      if(this.timer == null) {
        this.timer = setInterval( () => {
          ...
          getListInfo({rollingName : '上辊电机速度',rollingDeviceId:1}).then((res) =>{
            ...
            this.rollingTableData44[0].chartData.xData = []
            this.rollingTableData44[0].chartData.yData = []
            this.dataList.forEach(item =>{
              if(item.rollingValue>this.rollingTableData44[0].chartData.maxData){
              ...
              this.rollingTableData44[0].chartData.xData.push(item.rollingProduceTime)
              this.rollingTableData44[0].chartData.yData.push(item.rollingValue)
              this.rollingTableData44[0].chartData.rName= item.rollingName
            })
          })
          getListInfo({rollingName : '下辊电机速度',rollingDeviceId:1}).then((res) =>{
            this.dataList = res.data
            this.rollingTableData44[1].chartData.xData = []
            this.rollingTableData44[1].chartData.yData = []
            this.dataList.forEach(item =>{
              if(item.rollingValue>this.rollingTableData44[1].chartData.maxData){
                if(this.warnTable.length == 0 || this.dyTime[1] < item.rollingProduceTime){
                  this.warnTable.push(item);
                  this.dyTime[1] = item.rollingProduceTime
                }
              }
              this.rollingTableData44[1].chartData.xData.push(item.rollingProduceTime)
              this.rollingTableData44[1].chartData.yData.push(item.rollingValue)
              this.rollingTableData44[1].chartData.rName= item.rollingName
            })
          })
          getListInfo({rollingName : '主水泵电机速度',rollingDeviceId:1}).then((res) =>{
            this.dataList = res.data
            this.rollingTableData44[2].chartData.xData = []
            this.rollingTableData44[2].chartData.yData = []
            this.dataList.forEach(item =>{
              if(item.rollingValue>this.rollingTableData44[2].chartData.maxData){
                if(this.warnTable.length == 0 || this.dyTime[2] < item.rollingProduceTime){
                  this.warnTable.push(item);
                  this.dyTime[2] = item.rollingProduceTime
                }
              }
              this.rollingTableData44[2].chartData.xData.push(item.rollingProduceTime)
              this.rollingTableData44[2].chartData.yData.push(item.rollingValue)
              this.rollingTableData44[2].chartData.rName= item.rollingName
            })
          })
          getListInfo({rollingName : '备用水泵电机速度',rollingDeviceId:1}).then((res) =>{
            this.dataList = res.data
            this.rollingTableData44[3].chartData.xData = []
            this.rollingTableData44[3].chartData.yData = []
            this.dataList.forEach(item =>{
              if(item.rollingValue>this.rollingTableData44[3].chartData.maxData){
                if(this.warnTable.length == 0 || this.dyTime[3] < item.rollingProduceTime){
                  this.warnTable.push(item);
                  this.dyTime[3] = item.rollingProduceTime
                }
              }
              this.rollingTableData44[3].chartData.xData.push(item.rollingProduceTime)
              this.rollingTableData44[3].chartData.yData.push(item.rollingValue)
              this.rollingTableData44[3].chartData.rName= item.rollingName
            })
          })
          getListInfo({rollingName : '卷取电机速度',rollingDeviceId:1}).then((res) =>{
            this.dataList = res.data
            this.rollingTableData44[4].chartData.xData = []
            this.rollingTableData44[4].chartData.yData = []
            this.dataList.forEach(item =>{
              if(item.rollingValue>this.rollingTableData44[4].chartData.maxData){
                if(this.warnTable.length == 0 || this.dyTime[4] < item.rollingProduceTime){
                  this.warnTable.push(item);
                  this.dyTime[4] = item.rollingProduceTime
                }
              }
              this.rollingTableData44[4].chartData.xData.push(item.rollingProduceTime)
              this.rollingTableData44[4].chartData.yData.push(item.rollingValue)
              this.rollingTableData44[4].chartData.rName= item.rollingName
            })
          })
        }, 2000)
