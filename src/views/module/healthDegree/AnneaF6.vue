...
export default {
  ...
  async created() {
     ...
     * 获取一号铸轧机报警历史记录（30条）
     */
    await getDevice({ rollingDeviceNumber: '退火炉6#' }).then((res) => {
  ...
  methods: {
    ...
    getengineList() {
      ...
      //       退火炉6#
      // select * from t_51af30f0a62511ed8b62ff846936488a where ts > now - 5s;

      var zong = 'SELECT * FROM t_51af30f0a62511ed8b62ff846936488a where ts between'
    ...
    setTimer() {
      if (this.timer == null) {
        this.timer = setInterval(() => {
          ...
          // 定时查询退火炉最新20条报警记录
          getDevice({ rollingDeviceNumber: '退火炉6#' }).then((res) => {
            this.currentWarnTable = res.data
          })
        }, 3500)
      }
