...
//需要修改
import { getListNewData3, getListSpecial3, rollingOptions, rollingTableData1 } from '@/api/oneCastrollgy'
...
export default {
  ...
  async created() {
     ...
     * 获取一号铸轧机报警历史记录（30条）
     */
    await getTec({ rollingDeviceNumber: '铸轧机3#' }).then((res) => {
    ...
    setTimer() {
      if (this.timer == null) {
        this.timer = setInterval(() => {
          ...
          getTec({ rollingDeviceNumber: '铸轧机3#' }).then((res) => {
            this.currentWarnTable = res.data
          })
        }, 3500)
      }
