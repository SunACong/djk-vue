        ...
        <el-card shadow="always">
          <div slot="header" style="line-height: 20px;display: flex;justify-content: space-between;">
            <div style="display: flex;">
              ...
              <span style="line-height: 20px;">1#退火炉参数</span>
            </div>
          </div>
          <div>
...
import { getListNewData1, getListSpecial, getListSpecial1, rollingOptions, rollingTableData1 } from '@/api/AnneaF'
...
export default {
  ...
  async created() {
     ...
     * 获取一号铸轧机报警历史记录（30条）
     */
    await getListWarnHistoryData({ rollingDeviceNumber: '退火炉1#' }).then((res) => {
  ...
  methods: {
    ...
    getengineList() {
      ...
      //       退火炉1#
      // select * from t_26b106d0a62511ed8b62ff846936488a where ts > now - 5s;

      var zong = 'SELECT * FROM t_26b106d0a62511ed8b62ff846936488a where ts between'
    ...
    getMyData: function (index, row) {
       ...
       */
        ...
        getListSpecial1({ rollingName: row.rollingName, ts: row.rollingProduceTime }).then((res) => {
      ...
      if (this.timer == null) {
        this.timer = setInterval(() => {
          ...
          getListNewData1().then((res) => {
            ...
            // console.log('+++++++++++++++++', res.data[0].meterialT);
            // 1区炉气温度曲线
            this.rollingTableData1[3].value = res.data[0].zoneOneT.toFixed(1)
            // console.log('+++++++++++++++++', res.data[0].zoneOneT);
            // 2区炉气温度曲线  rollA
          ...
          // 定时查询退火炉最新20条报警记录
          getListWarnHistoryData({ rollingDeviceNumber: '退火炉1#' }).then((res) => {
