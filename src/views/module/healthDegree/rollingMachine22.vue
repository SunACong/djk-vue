...
import { getListNewData2, getListSpecial2, rollingOptions, rollingTableData1 } from '@/api/oneCastrollgy'
...
export default {
  ...
  async created() {
     ...
     * 获取一号铸轧机报警历史记录（30条）
     */
    await getTec({ rollingDeviceNumber: '铸轧机2#' }).then((res) => {
  ...
  methods: {
    ...
    getMyHistoryData() {
      ...
      getListDuringWarnData({ rollingDeviceNumber: '铸轧机2#', rollingName: this.indicatorName, begin, end }).then((res) => {
    ...
    setTimer() {
      if (this.timer == null) {
        ...
        getListNewData2().then((res) => {
          ...
          getTec({ rollingDeviceNumber: '铸轧机2#' }).then((res) => {
            this.currentWarnTable = res.data
          })
        }, 3500)
      }
